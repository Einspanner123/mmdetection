...
model = dict(
    ...
    backbone=dict(
        ...
        arch='b0',
        drop_path_rate=0.2,
        out_indices=(3,4,5,),  # Use multiple feature levels for FANG
        frozen_stages=0,
    ...
    neck=dict(
        type='LAMFPN',
        in_channels=[40, 112, 320],  # Match the channels from backbone out_indices (2,3,4,5)
        out_channels=64,
        num_outs=4,  # Match the number of scales for detection head
        start_level=0,
        norm_cfg=norm_cfg,
        apply_lam_levels=[1, 2],
        use_dw_conv=True,
        channel_reduction=2,  # Reduced from 4 to 2 to ensure channels are compatible with GroupNorm
